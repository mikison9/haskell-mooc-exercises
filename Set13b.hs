...
visit maze place = do
  ...
  if place `notElem` visited
    then do
      let newVisited = place : visited
      let toExplore = filter (`notElem` newVisited) (fromMaybe [] (lookup place maze))
      put newVisited >> mapM_ (visit maze) toExplore
    else do
      put visited
...
path maze place1 place2 = place2 `elem` reachable
  where
    reachable = execState (visit maze place1) []
...
instance Functor SL where
  -- implement fmap
  fmap f (SL c) = SL (\x -> case c x of (i, k, l) -> (f i, k, l))
...
instance Monad SL where
  -- implement return and >>=
  return a = SL (\s -> (a, s, []))
  (SL c1) >>= f = SL (\s -> case c1 s of (i, k, l) -> case f i of (SL c2) -> case c2 k of (i2, k2, l2) -> (i2, k2, l ++ l2))
