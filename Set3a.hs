...
firstHalf s = take toTake s
    where len = length s
          toTake = if odd len then (len `div` 2) + 1 else len `div` 2
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (\x -> x <= max && x > 0) [k^x | x <-[0..max]]
...
whileRight check x = case val of
        Left a -> a
...
joinToLength len strs = [cat | a <- strs, b <- strs, let cat = a ++ b, length cat == len]
...
sumRights list = sum $ map (fromRight 0) list
...
interpreter commands = recurse 0 0 commands []
    where recurse x y [] prints = prints
          recurse x y (command:commands) prints = case command of
            "up" -> recurse x (y + 1) commands prints
            "down" -> recurse x (y - 1) commands prints
            "left" -> recurse (x - 1) y commands prints
            "right" -> recurse (x + 1) y commands prints
            "printX" -> recurse x y commands (prints ++ [show x])
            "printY" -> recurse x y commands (prints ++ [show y])
