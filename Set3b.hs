...
buildList start count end = recurse start count end [end]
  where
    recurse _ 0 end list = list
    recurse start count end list = recurse start (count - 1) end (start : list)
...
sumsOf :: [Int] -> [Int]
sumsOf xs = recurse 0 xs
  where
    recurse i [] = []
    recurse i (x : xs) = (i + x) : recurse (i + x) xs
