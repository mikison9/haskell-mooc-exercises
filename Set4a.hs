...
distinct [] = True
distinct (x : xs) = if contains x xs then False else distinct xs
  where
    contains a [] = False
...
transfer from to amount bank =
  if fromExists && toExists && amount > 0 && fromBalance >= amount
    then Map.update (updateBalance amount) to (Map.update (updateBalance (-amount)) from bank)
    else bank
  where
    fromExists = Map.member from bank
    toExists = Map.member to bank
    fromBalance = Map.findWithDefault 0 from bank
    toBalance = Map.findWithDefault 0 to bank
    updateBalance y x = Just (x + y)
...
maxIndex arr = fst (helper (assocs arr))
  where
    helper (x : xs) = foldr (\(x, y) (a, b) -> if y > b then (x, y) else (a, b)) x xs
