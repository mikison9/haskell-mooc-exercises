...
instance Eq Name where
  ...
  (Name []) == _ = False
  _ == (Name []) = False
  (Name (x1 : x1s)) == (Name (x2 : x2s)) = if toLower x1 == toLower x2 then (Name x1s) == (Name x2s) else False
...
instance Num RationalNumber where
  ...
  (RationalNumber a b) + (RationalNumber c d) = simplify (RationalNumber (l * a + k * c) mult)
    where
      mult = lcm b d
      l = mult `div` b
      k = mult `div` d
