...
data State = Start | Error | Finished | AddedEggs | AddedFlour | AddedSugar | AddedSugarAndFlour | Mixed
  deriving (Eq, Show)
...
step AddedFlour AddSugar = AddedSugarAndFlour
step AddedSugar AddFlour = AddedSugarAndFlour
step AddedSugarAndFlour Mix = Mixed
...
average (a :| []) = a
average (a :| l) = (a + sum l) / fromIntegral len
  where
    len = length l + 1
...
reverseNonEmpty (a :| l) = head rev :| tail rev
  where
    rev = reverse l ++ [a]
...
passwordAllowed s (ContainsSome (l : ls)) = if l `elem` s then True else passwordAllowed s (ContainsSome ls)
passwordAllowed s (DoesNotContain l) = not (passwordAllowed s (ContainsSome l))
