...
blendColor (Color a1 a2 a3) (Color b1 b2 b3) = Color (avg a1 b1) (avg a2 b2) (avg a3 b3)
  where
    avg a b = (a + b) `div` 2
...
paintSolid color (Shape shape) (Picture pic) = Picture f
  where
    f coord = if shape coord then color else pic coord
...
mix :: [Color] -> Color
mix (c : cs) = case sum of (Color r g b) -> Color (r `div` len) (g `div` len) (b `div` len)
  where
    sum = foldr (\(Color a1 a2 a3) (Color b1 b2 b3) -> Color (a1 + b1) (a2 + b2) (a3 + b3)) c cs
    len = length cs + 1

blur :: Picture -> Coord -> Color
blur (Picture p) (Coord x y) = mix [c1, c2, c3, c4, c5]
  where
    c1 = p (Coord x y)
    c2 = p (Coord (x - 1) y)
    c3 = p (Coord (x + 1) y)
...
instance Transform Blur where
  apply _ base = Picture p
    where
      p coord = blur base coord
